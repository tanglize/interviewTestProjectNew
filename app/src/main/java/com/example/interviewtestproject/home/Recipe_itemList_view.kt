...
class RecipeItemListView(val recipeData: RecipeClass?): Item<ViewHolder>() {
    override fun bind(viewHolder: ViewHolder, position: Int) {
        Log.d("hobbyItem", "$position")
        viewHolder.itemView.itemListName.text = recipeData!!.recipeName

        Picasso.get().load("${recipeData.recipeImage}").into(viewHolder.itemView.imageView)
